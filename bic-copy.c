        ...
        while(start != end) {
            ...
            for(int currV = start; currV < oldEnd; currV++) {
                ...
                for(int currU = 0; currU < adjEnd; currU++) {
                    ...
                    if(P[u] == -1) {
                        // Set the P and L values using CAS in case another thread got in here too
                        if(__sync_bool_compare_and_swap(&P[u], -1, v) && __sync_bool_compare_and_swap(&L[u], -1, L[v] + 1)) {
...
void bicc(Graph *graph) {
    ...
    printf("i\tP\tL\n");
    for(int i = 0; i < numVertices; i = i + (numVertices * 0.25)) {
        printf("%d\t%d\t%d\n", i, P[i], L[i]);
    }
}
